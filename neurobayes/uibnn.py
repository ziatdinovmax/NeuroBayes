...
class UncertainInputBNN(BNN):
    ...
    def model(self, X: jnp.ndarray, y: jnp.ndarray = None, training: bool = True, **kwargs) -> None:
        """BNN probabilistic model"""

        # Sample input X
        X_prime = self._sample_input(X, training, **kwargs)
    ...
    def _sample_input(self, X: jnp.ndarray, training: bool = True, prng_key: jnp.ndarray = None) -> jnp.ndarray:
        """
        ...
        if training:
            ...
            X_prime = dist.Normal(X, sigma_x).sample(prng_key)
        return X_prime
    ...
    def sample_from_posterior(self,
        ...
        return predictive(rng_key, X_new, training=False, prng_key=rng_key)
